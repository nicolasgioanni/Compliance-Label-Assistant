...
5. Run verification and compare the result with the expected MVP result.

The current MVP verifies these fields:
...
The current MVP does not verify bottler/producer or country of origin. Those fields appear only as fixture context.
...
| Case | Image | Brand name | Class/type | Alcohol content | Net contents | Expected MVP result | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| TC07 | `images/tc07_class_type_mismatch.png` | `GOLDEN BARREL` | `Vodka` | `35% Alc./Vol. (70 Proof)` | `750 mL` | `fail` | Label shows `Spiced Rum`, so class/type should fail. |
...
4. Assert the current MVP overall status and field-level statuses.
