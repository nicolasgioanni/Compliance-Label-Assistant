...
| Deployed application URL | [deployment-links.md](deployment-links.md), [../../README.md](../../README.md) | Current value is `TBD before submission` because no deployed URL is present in tracked repository documentation or config. |
...
| Selected field verification | `backend/app/verification/rules.py` | Verifies brand name, class/type, alcohol content, net contents, and government warning text. |
| Simple UI for reviewers | `frontend/src/App.jsx`, `frontend/src/components/verification/VerificationForm.jsx`, `frontend/src/components/queue/` | UI centers on queue, expected data entry, result review, and export actions. |
