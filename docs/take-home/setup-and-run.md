...
This is the evaluator-facing setup path for running the prototype locally. The deployed app is available at https://compliance-label-assistant.vercel.app.
...
The setup script installs backend and frontend dependencies and initializes ignored local environment files from the checked-in examples unless `-NoEnvFile` is supplied.

Add the backend provider key to `backend\.env`:
...
| --- | --- | --- | --- |
| `VITE_API_BASE_URL` | Required for deployed frontend, optional locally because a code default exists | `http://127.0.0.1:8000` | Backend API base URL used by `frontend/src/api/verificationApi.js`. |

The checked-in example is `frontend/.env.example`. Do not put backend provider secrets in frontend environment files.
...
5. Upload a valid JPG/JPEG, PNG, WebP, or TIFF/TIF label image smaller than 5 MB.
6. Enter expected values for the selected label.
7. Run verification for the selected label.
8. Review the overall status, field statuses, extracted values, reasons, and timing fields.
...
| Upload is rejected | File type, MIME type, extension, size, or dimensions are outside configured limits | Use a JPG/JPEG, PNG, WebP, or TIFF/TIF image smaller than 5 MB. |
