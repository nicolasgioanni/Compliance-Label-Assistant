...
| `frontend/src/utils/resultExport.js` | CSV and Excel export utilities. | filename, row, CSV, XLSX, download helpers | `write-excel-file`, status resolution | `VerificationForm`, tests | Exports current results only and skips raw extracted text. |
| `frontend/src/utils/resultExport.test.js` | Tests export utilities. | none | Vitest, export utilities, mocked Excel writer | Test runner | Covers rows, filenames, CSV escaping, downloads, and XLSX rows. |
